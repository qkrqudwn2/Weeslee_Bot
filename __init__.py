...
def bread_conversation_G(question):
    response = openai.chat.completions.create(
        model="gpt-4o-mini", 
        messages=[
            {
                "role": "system", 
             "content": "당신은 제과제빵 전문가입니다. 친절하고 쉽게 요약하여 5가지 항목으로 답변해주세요"
             },
             {
                 "role": "user",
                 "content": question + " 만드는 법 알려줘"
             }
        ],
        max_tokens=512,
        n=1,
...
def bread_conversation_L(question):
    messages=[
            {
                "role": "system", 
             "content": "당신은 제과제빵 전문가입니다. 같은 말은 반복하지 않고 친절하고 쉽게 요약하여 5가지 항목으로 답변해주세요"
             },
             {
                 "role": "user",
                 "content": question + " 만드는 법 알려줘"
